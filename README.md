...
- **Login (and `/ready`) hang indefinitely with no response, while `/health` responds instantly:** `/health` doesn't touch MongoDB; `/ready` and `POST /api/auth/login` both do. A hang (rather than a fast `503`) means the API can reach *some* address for Mongo but the connection is stalling — e.g. a stale/unreachable private hostname, or the linked `mongodb` service is suspended/crashed. Check that a `mongodb` service exists in the project, is running, and is actually linked to the API service's Mongo connection variable (Zeabur may inject this as a referenced/auto-generated variable rather than one you typed by hand — check for it under the API service's Variables page). Restart the `mongodb` service and redeploy the API if needed.
