  ...
  return (
    <DashboardLayout>
      <div className="p-6" data-testid="certificates-page">
        <PageHeader overline={t("nav.certificates")} title={t("certificate.myCertificates")} />
            ...
            {certificates.map((cert) => (
              <Card key={cert.id} className="bg-white border border-slate-200 rounded-sm overflow-hidden" data-testid={`certificate-${cert.id}`}>
                ...
                >
                  ...
                  <p className="text-sm text-slate-500 mt-4">{t("quiz.yourScore")}: {cert.score}%</p>
                </div>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-slate-500">{t("adminCertificates.certificateId")}: {cert.certificate_id}</p>
                      <p className="text-xs text-slate-500">
                        {t("certificate.issuedOn")}: {new Date(cert.issued_at).toLocaleDateString()}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2 justify-end">
                      <Button
                        ...
                        data-testid={`verify-cert-${cert.id}`}
                      >
                        <Link to={`/certificates/verify/${cert.certificate_id}`}>
                      ...
                      >
                        <Eye className="w-4 h-4 mr-2" /> {t("certificate.view")}
                      </Button>
                      <Button
                        variant="outline"
                        className="rounded-sm"
