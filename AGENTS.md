...
- **Frontend local-dev can use same-origin `/api` (recommended).** Leave `REACT_APP_BACKEND_URL` unset so the browser calls `/api` and the CRACO proxy forwards to the backend (`BACKEND_PROXY_URL`, default `http://localhost:8001`). The proxy’s `onProxyReq` rewrites JSON bodies that `@emergentbase/visual-edits` consumes in dev, so login/refresh POSTs do not hang. Optional: set `REACT_APP_BACKEND_URL=http://localhost:8001` for direct cross-origin API calls (requires restart of `yarn start` after changing `frontend/.env`).
